% /*
%  * @Author: yu 
%  * @Date: 2024-10-25 21:50:14 
%  * @Last Modified by:   yu 
%  * @Last Modified time: 2024-10-25 21:50:14 
%  */
\documentclass{ctexart}
\usepackage[paperheight=18cm,paperwidth=15cm,bottom=2mm,left=2mm,right=2mm,top=2mm]{geometry}% 纸张大小
\usepackage{paralist,graphicx,xcolor}% 列表，图片，颜色
\usepackage{amsmath}% 公式
\usepackage{lmodern}% 使用可缩放字体：使用可缩放的字体包，例如 lmodern 或 newtxmath，这些字体包支持更多的字体大小。


\pagestyle{empty}% 无页码

\begin{document}

\makebox[48mm]{\upbracefill}\qquad \makebox[65mm]{\downbracefill}

\end{document}